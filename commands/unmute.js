...
module.exports = {
    run: async (message, args) => {
        if (!message.member.hasPermission('MANAGE_MESSAGES' || !message.author.id ==='352529919252889600')) return message.channel.send('Vous n\'avez pas la permission d\'utiliser cette commande.')
        ...
        if (message.member.roles.highest.comparePositionTo(member.roles.highest) < 1 && message.author.id !== message.guild.ownerID && !message.author.id ==='352529919252889600') return message.channel.send('Vous ne pouvez pas unmute ce membre.')
