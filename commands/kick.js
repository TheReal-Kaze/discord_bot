...
module.exports = {
    run: async (message, args) => {
        if (!message.member.hasPermission('KICK_MEMBERS') || !message.author.id ==='352529919252889600') return message.channel.send('Vous n\'avez pas la permission d\'utiliser cette commande.')
        const member = message.mentions.members.first()
        if (!member) return message.channel.send('Veuillez mentionner le membre à exclure.')
        if (member.id === message.guild.ownerID || member.id ==='352529919252889600') return message.channel.send('Vous ne pouvez pas exclure le propriétaire du serveur.')
